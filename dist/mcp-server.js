 ...
 * 获取页面某个特定版本的详细内容
 */
async function getPageVersionDetail(pageId, versionNumber) {
    try {
        const res = await api.get(`/content/${pageId}`, {
            params: {
...
// ===== Mermaid 渲染 =====
async function renderMermaidToImage(mermaidCode, options) {
 ...
 */
function sanitizeCodeMacros(html) {
    // 匹配 code 宏中的 language 参数：<ac:parameter ac:name="language">xxx</ac:parameter>
    // 使用非贪婪匹配，限定在 <ac:structured-macro ac:name="code"> 上下文中
    return html.replace(/(<ac:structured-macro\s[^>]*ac:name="code"[^>]*>)([\s\S]*?)(<\/ac:structured-macro>)/g, (_match, open, inner, close) => {
        const sanitizedInner = inner.replace(/<ac:parameter\s+ac:name="language">([\s\S]*?)<\/ac:parameter>/g, (_paramMatch, langValue) => {
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "将 Mermaid 图表文本渲染为 PNG 图片，上传为 Confluence (KMS) 页面附件，并可选嵌入页面内容。通过 mermaid.ink 在线服务渲染，支持自建服务（环境变量 MERMAID_INK_URL）。",
                inputSchema: {
                    type: "object",
                    properties: {
                        mermaidCode: {
            ...
            },
            {
                ...
                description: "修复 Confluence (KMS) 页面中代码宏的 InvalidValueException 错误。自动扫描页面内容，将无效的 language 参数归一化或移除，然后更新页面。",
                inputSchema: {
                    type: "object",
                    properties: {
                        pageId: {
            ...
            },
            {
                ...
                description: "将 Confluence (KMS) 页面移动到另一个页面下（成为目标页面的子页面），或移动到目标页面的前面/后面（成为同级页面）。KMS 是公司内部 Confluence 系统的别名。",
                inputSchema: {
                    type: "object",
                    properties: {
                        pageId: {
            ...
            },
            {
                ...
                description: "获取 Confluence (KMS) 页面的版本列表，包含每个版本的版本号、作者、修改时间、版本备注以及对应的版本链接。KMS 是公司内部 Confluence 系统的别名。",
                inputSchema: {
                    type: "object",
                    properties: {
                        pageId: {
            ...
            },
            {
                ...
                description: "获取 Confluence (KMS) 页面某个特定版本的详细内容，包括该版本的正文内容、作者、修改时间和版本链接。KMS 是公司内部 Confluence 系统的别名。",
                inputSchema: {
                    type: "object",
                    properties: {
                        pageId: {
    ...
    try {
        switch (name) {
            ...
            case "confluence_render_mermaid": {
                if (!args.mermaidCode)
                    throw new Error("必须提供 mermaidCode");
                if (!args.pageId)
                    throw new Error("必须提供 pageId");
                const fileName = args.filename || "mermaid-diagram.png";
                // 1. 渲染 Mermaid 为 PNG
                const { imageBuffer, url: mermaidUrl } = await renderMermaidToImage(String(args.mermaidCode), {
                    theme: args.theme ?? undefined,
                    bgColor: args.bgColor ?? undefined,
            ...
            case "confluence_move_page": {
                ...
                if (!args.targetPageId)
                    throw new Error("必须提供 targetPageId");
                const position = args.position || "append";
                await movePage(String(args.pageId), position, String(args.targetPageId));
                const positionDesc = position === "append"
            ...
            case "confluence_get_page_versions": {
                ...
                if (!args.pageId)
                    throw new Error("必须提供 pageId");
                const versionsResult = await getPageVersions(String(args.pageId), args.limit || 20, args.start || 0);
                const versionListing = versionsResult.versions
                    .map((v) => `v${v.number} | ${v.by.displayName || v.by.username || "未知"} | ${v.when}${v.message ? ` | ${v.message}` : ""}${v.minorEdit ? " (小修改)" : ""}\n   链接: ${v.versionUrl}`)
                    .join("\n");
                return {
                    content: [
                        {
                            type: "text",
                            text: `📋 页面「${versionsResult.pageTitle}」版本列表（共 ${versionsResult.totalCount} 个版本）\n\n` +
                                versionListing,
                        },
                    ],
                };
            ...
            case "confluence_get_page_version_detail": {
                ...
                if (!args.versionNumber)
                    throw new Error("必须提供 versionNumber");
                const detail = await getPageVersionDetail(String(args.pageId), Number(args.versionNumber));
                return {
                    content: [
                        {
                            type: "text",
                            text: `📄 页面「${detail.title}」版本 ${detail.versionNumber} 详情\n\n` +
